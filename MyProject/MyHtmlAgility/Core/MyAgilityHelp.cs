...
{
    public static class MyAgilityHelp
    {
        public static HtmlAgilityPack.HtmlDocument GetHtml()
        {
            var doc = new HtmlAgilityPack.HtmlDocument();
            return doc;
        }

        public static CookieCollection GetCookieCollection(this string str, string defaultDomain="")
        {
            ...
            str.Split(';').Select(a =>
              {
                  ...
                  {
                      Name = a.Substring(0, falg).Trim(),
                      Value = a.Substring(falg + 1, a.Length - falg - 1).Trim().DealAsCookie(),
                      Domain = defaultDomain
                  };
                  return cookie;
              }).ToList().ForEach(cookies.Add);
            return cookies;
        }

        public static string DealAsCookie(this string str)
        {
            str = str.Replace(",", "%2C");
            return str;
        }
